...
function touchend(_: TouchEvent) {
	button = false;
}
...
export default function Home() {
	...
	let [messages, setMessages] = useState([
		{
			user: false,
			text: "Hi! I am your helpful BU assistant for mental health. How can I help you today?",
		},
	]);
		...
		<div className="main">
			...
			<div className="body">
				<div className={"body-main " + poppins.className}>
					...
					{isSystemThinking && (
						<div className="response">Assistant thinking...</div>
					)}
				</div>
			</div>
...
async function initialize(canvas: HTMLCanvasElement) {
	...
	let dimensionsBuffer = new Buffer(
		device,
		BufferFormat.U32,
		GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
		2
	);
	let densityDimensionsBuffer = new Buffer(
		device,
		BufferFormat.U32,
		GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
		2
	...
	let pipeline = new RenderPipeline(
		device,
		new Shader(device, textureCode),
		device.format,
		[{ format: VertexFormat.F32_2 }]
	);
	let pass = new RenderPass(
		...
		[
			[
				{
					i: 0,
					resource: new Sampler(device, { mag: SamplerFilterMode.LINEAR }),
				},
				{ i: 1, resource: texture },
			],
		],
		[vertexBuffer]
	);
	...
	let velocityBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC,
		w * h * 2
	);
	let previousVelocityBuffer = new Buffer(
		device,
		BufferFormat.F32,
	...
	let divergenceBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.STORAGE,
		w * h
	);
	let curlBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.STORAGE,
		w * h
	...
	let pressureBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC,
		w * h
	);
	let previousPressureBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
		w * h
	...
	let densityBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC,
		width * height * 4
	);
	let previousDensityBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
		width * height * 4
	...
	let positionBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
		2
	);
	let impulseBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
		2
	);
	let colorBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
		3
	);
	let radiusBuffer = new Buffer(
		device,
		BufferFormat.F32,
		GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
		1
	...
	let advectionPass = new ComputePass(
		new ComputePipeline(device, shader, "advection"),
		[
			[bufferConfig.dimensions],
			[bufferConfig.velocity, bufferConfig.previousVelocity],
		]
	);

	let calculateDivergencePass = new ComputePass(
		new ComputePipeline(device, shader, "calculateDivergence"),
		[
			[bufferConfig.dimensions],
			[bufferConfig.velocity, bufferConfig.divergence],
		]
	);
	let calculateCurlPass = new ComputePass(
		new ComputePipeline(device, shader, "calculateCurl"),
		[[bufferConfig.dimensions], [bufferConfig.velocity, bufferConfig.curl]]
	);
	let vorticityPass = new ComputePass(
		new ComputePipeline(device, shader, "vorticity"),
		[[bufferConfig.dimensions], [bufferConfig.velocity, bufferConfig.curl]]
	);

	let jacobiPass = new ComputePass(
		new ComputePipeline(device, shader, "jacobi"),
		[
			[bufferConfig.dimensions],
			[
				bufferConfig.divergence,
				bufferConfig.pressure,
				bufferConfig.previousPressure,
			],
		]
	);
	let gradientSubtractionPass = new ComputePass(
		new ComputePipeline(device, shader, "gradientSubtraction"),
		[[bufferConfig.dimensions], [bufferConfig.velocity, bufferConfig.pressure]]
	);

	let densityAdvectionPass = new ComputePass(
		new ComputePipeline(device, shader, "densityAdvection"),
		[
			[bufferConfig.dimensions, bufferConfig.densityDimensions],
			[
				bufferConfig.velocity,
				bufferConfig.density,
				bufferConfig.previousDensity,
			],
		]
	);
	let transferPass = new ComputePass(
		new ComputePipeline(device, shader, "transfer"),
		[
			[bufferConfig.texture, bufferConfig.densityDimensions],
			[bufferConfig.density],
		]
	);

	let addImpulsePass = new ComputePass(
		new ComputePipeline(device, shader, "addImpulse"),
		[
			[bufferConfig.dimensions],
			[bufferConfig.velocity],
			[bufferConfig.position, bufferConfig.impulse, bufferConfig.radius],
		]
	);
	let addDensityPass = new ComputePass(
		new ComputePipeline(device, shader, "addDensity"),
		[
			[bufferConfig.densityDimensions],
			[bufferConfig.density],
			[bufferConfig.position, bufferConfig.color, bufferConfig.radius],
		]
	);

	let x = Math.ceil(w / 8),
		y = Math.ceil(h / 8);
	let nx = Math.ceil(width / 8),
		ny = Math.ceil(height / 8);
	...
	function loop() {
		if (!running) {
			device.beginPass(device.texture, {
		...
		let position = mouse;
		if (button) {
			let localPosition = new Vector2(
				(position.x * w) / canvas.width,
				h - (position.y * h) / canvas.height
			);
			let localPrevious = new Vector2(
				(previous.x * w) / canvas.width,
				h - (previous.y * h) / canvas.height
			);
